...
/** "First Last", falling back to the email when both name fields are blank
 * (a user hasn't set one, or - for KnowledgeAuthor - never will). Repeated
 * inline as `[x.first_name, x.last_name].filter(Boolean).join(" ") || x.email`
 * across TopBar, ArticleCard, QuestionCard, and the article/question detail
 * pages before being pulled out here. */
