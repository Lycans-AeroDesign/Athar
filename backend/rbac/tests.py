...
class PermissionEnforcementTests(APITestCase):
    ...
    def test_user_without_permission_is_forbidden(self):
        ...
        user = User.objects.create_user(email="plain@example.com", password="password123")
    ...
    def test_granting_permission_to_role_unlocks_access_with_no_code_change(self):
        guest_role = Role.objects.get(name="Guest")
        user = User.objects.create_user(email="plain2@example.com", password="password123")
        guest_role.user_roles.create(user=user)
        access = self.client.post(
            reverse("auth-login"),
    ...
    def test_role_permission_crud_is_audited(self):
        ...
        admin = User.objects.create_superuser(email="admin2@example.com", password="password123")
        access = self.client.post(
            reverse("auth-login"),
